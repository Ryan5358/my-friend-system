<?php 
    ...
    $err_db = "";
    if(isset($_GET["addfriend_id"]) && !empty($_GET["addfriend_id"])) {
        $add_id = $_GET["addfriend_id"];
        // execute add friend actions

        // add record to myfriends table
        $db["myfriends"]->addRecords([[$_SESSION["id"], $add_id]]);
...
<section class="full content body layout-1 bg-1">
        <div class="container-fluid">
            <div class="row gx-5 d-flex justify-content-center align-items-center">
                ...
                <form class="col-md-8 d-flex flex-column justify-content-center align-items-center mt-3" action="friendadd.php" method="get">
                    <div class="container glassmorphism side right rounded-4">
                        ...
                        <div class="row gy-3 gx-3 justify-content-center">
                            <?php
                                // print cards
                                $pFriendAdd = new Paginator(5, $nonfriend_ids);
                                $pFriendAdd->set_cur_page($pageNum);
                                $friend_ids_page = $pFriendAdd->get_page();
                                $friends = array();
                                echo (empty($friend_ids_page) ? "<p class='fs-2 text-center'>You have become friends with everyone!</p>" : "");
                                ...
                                foreach($friends as $friend) {
                                    $friend->print_card_addfriend();
                                }
                            ?>
                        </div>
                    </div>
                    <?= $pFriendAdd->print_pagination() ?>
                </form>
            </div>
        </div>
    </section>
